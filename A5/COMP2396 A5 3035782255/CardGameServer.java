...
public class CardGameServer {
	...
	public void start(int port) {
		...
		try {
			...
			if (!silentMode) ex.printStackTrace();
		}
	} // start
	...
	private synchronized void addConnection(Socket clientSocket) {
		...
		if (numOfPlayers < maxNumOfPlayers) {
			...
			for (int i = 0; i < maxNumOfPlayers; i++) {
				if (clientSockets[i] == null) {
					try {
						...
						numOfPlayers++;
						println("Establishes a connection with a client at " + clientSocket.getRemoteSocketAddress());
						...
						oostream.writeObject(new CardGameMessage(CardGameMessage.PLAYER_LIST, i, clientNames));
					} catch (Exception ex) {
			...
			println("Server is full: cannot establish a connection with a client at " + clientSocket.getRemoteSocketAddress());
